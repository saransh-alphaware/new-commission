  ...
  const approveAction = [
    {
      ...
      data: (data) => {
        navigate("/daily-deposit-account/view", {
          state: {
            endpoint: `saving-accounts/particular/ddsAccount/${data?.id}`,
          },
        });
      },
    },
    ...
    <div className="flex flex-col mx-4 text-black dark:text-white">
      <div className="mt-3 flex w-full space-y-2 flex-col md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col">
          <button
            type="button"
            onClick={() => navigate("/daily-deposit-account/create")}
          ...
          >
            ...
            Create New DDS Deposit Account
          </button>
        </div>
      </div>
      <CheckerMakerTab
        accountType="DDS_ACCOUNT"
        eventType={"BRANCH_SCHEME_DAILY_DEPOSIT_ACCOUNT"}
...
export default DDSDepositAccountList;
