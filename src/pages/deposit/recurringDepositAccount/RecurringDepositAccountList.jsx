...
const RecurringDepositAccountList = () => {
  ...
  const approveAction = [
    {
      ...
      data: (data) => {
        navigate("/recurring-deposit-account/view", { state: { endpoint: `saving-accounts/particular/recurringAccounts/${data?.id}` } });
      },
    },
  ];
  return (
    <div className="flex flex-col mx-4 text-black dark:text-white">
      <div className="mt-3 flex w-full space-y-2 flex-col md:flex-row md:items-center md:justify-between">
        ...
        <div className="flex flex-col">
          <button
            type="button"
            onClick={() => navigate("/recurring-deposit-account/create")}
      ...
      <CheckerMakerTab
        accountType="RECURRING_DEPOSIT_ACCOUNT"
        eventType={"BRANCH_RD_ACCOUNT"}
...
export default RecurringDepositAccountList;
