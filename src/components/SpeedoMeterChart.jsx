...
const SpeedoMeterChart = ({ dateTime }) => {
  ...
  const getData = async (options) => {
    try {
      ...
      const response = await getServerData(
        `agents/target-achieved-group`,
        {
          creationDate: dateTime?.year?.[0],
          ...
          promotionalStartDate: dateTime?.year?.[0],
          promotionalEndDate: dateTime?.year?.[1],
          agentId: agentId,
          target: "group",
        },
        options
